...
export async function lunch(body:DataList) {
  // 注意这是一个拼接字符串的实例,
  //.query 后是 gql 查询的的名字，在 schema 中定义
  // 这个输出展示了查询的结构，但不会直接把 loginName 和 loginPassword 的值替换进去。
  const query = gql`
    query usergetFoudlist($params: DataList) {
      usergetFoudlist(params: $params)
    }
  `;
  ...
  const data = {
    ...
    operationName: 'usergetFoudlist', // 操作名称，选填，查询文档有多个操作时必填
    variables, // 对象集合，选填
  };
  ...
  return request<API.ResponseData>('/graphql', {
    ...
    .then((response) => {
      ...
      // 把 account 返回
      return response.data.usergetFoudlist;
      // }
